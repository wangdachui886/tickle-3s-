...
> 目前是早期体验版，核心记账、小组件、统计和 CSV 备份流程已经可用，适合试用和小范围测试。
...
Tickle 轻记尽量少要权限。当前 `AndroidManifest.xml` 只声明了 `RECEIVE_BOOT_COMPLETED`，用于手机重启后恢复和刷新桌面小组件。

| 权限类型 | 是否需要 | 说明 |
| --- | --- | --- |
| 网络权限 | 不需要 | 未声明 `INTERNET`，默认不联网 |
| 登录账号 | 不需要 | 不需要注册，也没有账号系统 |
| 银行卡 / 金融账户 | 不需要 | 不绑定任何账户 |
| 短信 | 不需要 | 不读取短信账单 |
| 相册 | 不需要 | 不扫描截图 |
| 通讯录 | 不需要 | 不读取联系人 |
| 定位 | 不需要 | 不记录消费地点 |
| 通知读取 | 不需要 | 不依赖通知识别账单 |
| 开机完成 | 需要 | 用于手机重启后刷新桌面小组件 |
